...
/**
 * Coaches que piden dar una clase que el gimnasio todavía no asignó.
 *
 * Aprobar **asigna la clase** al coach y cierra lo demás que colgaba de ella (las
  ...
  return (
    <Stack gap="md">
      <Group gap="sm">
        ...
        {filas.length > 0 ? <CountBadge count={filas.length} /> : null}
      </Group>
      <Text size="sm" c="dimmed">
      ...
      {solicitudes.isLoading ? (
        ...
        filas.map((s) => <Solicitud key={s.id} s={s} gymId={gymId} />)
      )}
    </Stack>
  );
}
